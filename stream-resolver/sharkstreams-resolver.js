...
async function resolve(result, teamName1, teamName2) {
  ...
  console.log('Resolving sharkstreams Stream')
  try {
    let res = await axios.get('https://sportslivetv.info/category/nba')
    // Find url

    let matchUrl
    let $ = cheerio.load(res.data)
    $('a.card').each(function(i, aa){
    ...
    result.sharkstreams = $('iframe').first().attr('src')

  } catch(error) {
    console.log(`Failed to resolve sharkstreams, error is: ${error}`)
  }
